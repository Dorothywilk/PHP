        ...
        <li class="guestBook-entry<?php   if ($c->getVersionObject()->getVersionAuthorUserName() == $u->getUserName()) {?> authorPost <?php   }?>">
            ...
            <?php   if($bp->canWrite()) { ?>
                <div class="btn-toolbar pull-right">
                    <div class="btn-group">
                        <a class="btn btn-success btn-mini" title="<?php  echo t('Edit')?>" href="<?php  echo $this->action('loadEntry')."&entryID=".$p['entryID'];?>#guestBookForm"><i class="icon-pencil icon-white"></i></a> |
                        <a class="btn btn-warning btn-mini" title="<?php  echo t('Remove')?>" href="<?php  echo $this->action('removeEntry')."&entryID=".$p['entryID'];?>" onclick="return confirm('<?php  echo t("Are you sure you would like to remove this comment?")?>');"><i class="icon-trash icon-white"></i></a> |
                        <?php   if($p['approved']) { ?>
                            <a class="btn btn-danger btn-mini" title="<?php  echo t('Un-Approve')?>" href="<?php  echo $this->action('unApproveEntry')."&entryID=".$p['entryID'];?>"><i class="icon-remove-sign icon-white"></i></a>
                        <?php   } else { ?>
                            <a class="btn btn-primary btn-mini" title="<?php  echo t('Approve')?>" href="<?php  echo $this->action('approveEntry')."&entryID=".$p['entryID'];?>"><i class="icon-ok-sign icon-white"></i></a>
                        <?php   } ?>
                    </div>
                </div>
            <?php   } ?>
                ...
                <?php   } ?>
                    ...
                    <?php   if(!$controller->authenticationRequired){ ?>
                        <div class="control-group <?php  echo (isset($errors['name'])?"error":"")?>">
                            <label class="control-label" for="name"><i class="text-error">* </i><?php  echo t('Name')?>:</label>
                            <div class="controls">
                                <input required="required" type="text" name="name" value="<?php  echo $Entry->user_name ?>" />
                        ...
                        <div class="control-group <?php  echo (isset($errors['email'])?"error":"")?>">
                            <label class="control-label" for="email"><i class="text-error">* </i><?php  echo t('Email')?>:</label>
                            <div class="controls">
                                <input required="required" type="email" name="email" value="<?php  echo $Entry->user_email ?>" />
                    ...
                    <div class="control-group <?php  echo (isset($errors['commentText'])?"error":"")?>">
                        <label class="control-label" for="name"><i class="text-error">* </i><?php  echo t('Comment')?>:</label>
                        <div class="controls">
                            <textarea class="span4" rows="6" required="required" name="commentText"><?php  echo $Entry->commentText ?></textarea>
                    ...
                    <?php 
                        if($controller->displayCaptcha) {
                            ...
                            <div class="control-group captcha">
                                <label class="control-label"><i class="text-error">* </i><?php  echo t('Please type the letters and numbers shown in the image.'); ?></label>
                                <div class="controls">
                                     <?php   $captcha->display(); ?>
