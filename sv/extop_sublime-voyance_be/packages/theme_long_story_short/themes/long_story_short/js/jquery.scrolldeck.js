...
(function($) {
    $.scrolldeck = function(options) {
		...
		var init = function() {
			...
			scrolldeck.controller = $.scrollorama({blocks:slides, offset:scrolldeck.settings.offset, enablePin:false});
			...
			$(document).on('keydown', function(e){
				...
				if (e.keyCode == 37 || e.keyCode == 38) {
					var slide = $($('ul.nav').find('li.active').find('a').attr('href'));
					currIndex = slide.index();
					if (currIndex > 0) {
						var slide = $('.slide').eq(currIndex-1);
					}
					scrollToSlide(slide);
				}
