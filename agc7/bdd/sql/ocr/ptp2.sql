...
SELECT a.id, DATE_FORMAT(a.date_publication, '%d/%m/%Y %h:%i') AS date_publication, u.pseudo, a.titre, a.resume
FROM Article AS a
INNER JOIN Utilisateur AS u ON a.auteur_id = u.id
...
SELECT a.id, DATE_FORMAT(a.date_publication, '%d %M %Y à %h heures %i') AS date_publication, u.pseudo, a.titre, a.contenu, GROUP_CONCAT(c.nom SEPARATOR ', ')
FROM Article AS a
INNER JOIN Utilisateur AS u ON a.auteur_id = u.id
INNER JOIN Categorie_article AS ca ON ca.article_id = a.id 
